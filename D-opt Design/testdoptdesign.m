...
% Last: solve the problem by using the new quantum relaxation and rounding
    ...
    cvx_begin
        ...
        minimize quantum_rel_entr(Y, A_mat*diag(z)*A_mat'+eps*eye(n_mat))-(n_mat-trace(Y))*log(eps)
      t1 = toc(tStart);
      cvx_end
      ...
      ofv_dual_quant=-quantum_rel_entr(Y, A_mat*diag(z)*A_mat'+eps*eye(n_mat))+(n_mat-trace(Y))*log(eps);
